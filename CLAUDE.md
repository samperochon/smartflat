...
- **`features/`**: Feature extraction and analysis pipelines
  ...
  - `symbolic_barycenter/`: TWE distance + DBA barycenter averaging. The former junk-drawer `baselines.py` was split (arc-audit Phase 2, RESULTS_HANDOFF §23) into `distances.py` (rTWE `RTWE_NU`/`RTWE_LMBDA` constants, embed/decode, DTW/soft-DTW primitives, `dist_*`/`pmatch_*`), `builders.py` (the ~17 `barycenter_*` constructors — DBA/Soft-DTW/SSG/FGW/MSA-consensus + discreteness-fairness lever variants), `registries.py` (the six `*_methods` `{build, distance}` registries), and `evaluation.py` (CV helpers, `evaluate_baselines`/`evaluate_incremental_ordering`, significance tests, bootstrap CIs); `baselines.py` is now a thin **back-compat re-export shim** (so `from …baselines import X` still resolves for every X). Plus `barycenter_quality.py` (method-agnostic representation-quality harness, RESULTS_HANDOFF §17–§20); `order_evaluation.py` (frequency-preserving order-shuffle-null ΔAUC, §15); `structure_metrics.py` (reusable per-sequence behavioral-structure metrics — perseveration, fragmentation, LZ complexity, drift — + leakage-guarded group analysis); `vocab.py` (shared G=28 cohort + ground-cost loaders)
