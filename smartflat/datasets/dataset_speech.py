"""Smartflat Speech recognition and representation."""
import os
import sys
...
class SpeechDataset(SmartflatDatasetBase):
    """Dataset class for the speech recognition and representation."""

    def __init__(
        self,
        dataset_name: str = 'speech_recognition_representation',
        root_dir: str = None,
    ...
    def load_metadata(self) -> pd.DataFrame:
        ...
        metadata['flag'] = metadata.apply(lambda x: 'success' if (x.speech_recognition_computed and x.speech_representation_computed) else x.flag_speech_representation, axis=1)
        
        # Filter to take audio modality
        metadata = metadata[metadata['modality'] == metadata['audio_modality']]     
    ...
    def __getitem__(self, index):
        ...
        path = meta_row.skeleton_landmarks_path
    ...
    def define_cohort(self, metadata, scenario):
        ...
        if 'success' in scenario:
            ...
            #assert (metadata["num_identifier_per_modality"] == 1).all()
            if not (metadata["num_identifier_per_modality"] == 1).all():
                print('[WARNING] (metadata["num_identifier_per_modality"] == 1).all()')
            print("Testing...")
...
if __name__ == "__main__":
    ...
    print(f"Benchmarking dataset: '{SpeechDataset.__name__}'")
    dset = SpeechDataset(root_dir=get_data_root(), scenario='all')
