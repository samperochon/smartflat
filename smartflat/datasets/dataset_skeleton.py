...
class SkeletonDataset(SmartflatDatasetBase):
    """Dataset class for the skeleton (pose) landmarks estimation."""

    def __init__(
        self,
        dataset_name = 'skeleton_landmarks',
        root_dir: str = None,
    ...
    def load_metadata(self) -> pd.DataFrame:
        ...
        metadata['flag'] = metadata.apply(lambda x: 'success' if x.skeleton_landmarks_computed else x.flag_skeleton_landmarks, axis=1)
...
if __name__ == "__main__":
    ...
    print(f"Benchmarking dataset: '{SkeletonDataset.__name__}'")
    dset = SkeletonDataset(root_dir=get_data_root(), scenario='all')
